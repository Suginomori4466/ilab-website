 ...
 * 定数の定義
 */
$JSON_SUCCESS = json_encode(["success" => true]);
$JSON_FAILURE = json_encode(["success" => false]);
...
/*
 * CSSファイルをHTML形式に直して返す．
 */
function CreateCSSList($css)
{
  $result = '';
...
/*
 * JavaScriptファイルをHTML形式に直して返す．
 */
function CreateJSList($js)
{
  $result = '';
  foreach ($js as $value) {
    $result .= '<script src="'.$value.'" charset="utf-8"></script>';
  }
  return $result;
}
